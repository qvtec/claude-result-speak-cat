...
- **macOS 対応** — Python 3 + AppKit（PyObjC）で動作、追加インストール不要
...
| macOS | Xcode Command Line Tools（`/usr/bin/python3` 経由でPyObjCを利用） |

Xcode CLT は開発者Macには標準的に入っています。未インストールの場合：
...
| 環境 | 動作 |
|------|------|
| `/usr/bin/python3` + PyObjC（Xcode CLT） | 猫画像 + アニメーション表示 |
| ユーザーの `python3` + tkinter | 猫画像 + アニメーション表示 |
| Python が見つからない | `osascript` でシステム通知 |

`pip install` も `brew install` も不要です。
